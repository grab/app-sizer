...
import com.grab.tools.utils.DEFAULT_AAR_FOLDER
...
class FeatureAnalyticReport(
    ...
    override fun report(androidBinaryInfo: Set<ApkFileInfo>, contributor: Set<Contributor>) {
        val moduleToContributorMap = contributor.moduleToContributor()
        ...
        featureToContributorMap.forEach { t, u ->
            println("-------Feature : $t")
            u.forEach {
                println(it.path)
            }
...
//    private fun report(apks: Set<ApkFileInfo>, contributor: Map<String, List<Contributor>>) {
...
private class Feature(
    ...
    val assets: Set<RawFileInfo>  = contributor.flatMap { it.assets }.toSet()
